                ...
                <ColumnsDirective>
                    <ColumnDirective field='name' headerText='Name' width='200' textAlign='Left' template={(props: UserData) => (
                        <div className='flex items-center gap-1.5'>
                            <img src={props.imageUrl || '/assets/icons/user-profile.png'} alt='user' className='rounded-full size-8 aspect-square' referrerPolicy='no-referrer'/>
                            <span>{props.name}</span>
                        </div>
                    )}/>
                    <ColumnDirective field='email' headerText='Email' width='200' textAlign='Left' />
                    <ColumnDirective field='joinedAt' headerText='Date Joined' width='140' textAlign='Left' template={({joinedAt}: {
                        joinedAt: string
                    }) => formatDate(joinedAt)}/>
                    <ColumnDirective field='status' headerText='Role' width='100' textAlign='Left' template={({ status }: UserData) => (
