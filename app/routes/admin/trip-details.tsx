            ...
            <section className='container wrapper-md'>
                <header>
                    ...
                    <div className='flex items-center gap-5'>
                        ...
                        <InfoPill text={itinerary?.slice(0, 5).map((item) => item.location).join(', ') || ''}
                                  image="/assets/icons/location-mark.svg"/>
                    </div>
                </header>
                <section className='gallery'>
                    {imageUrls.map((url: string, index: number) => (
                        <img key={index} src={url} alt={`image-${index}`} className={cn('w-full rounded-xl object-cover', index === 0 ? 'md:col-span-2 md:row-span-2 h-[330px]' : 'md:row-span-1 h-[150px]')}/>
                    ))}
                </section>
